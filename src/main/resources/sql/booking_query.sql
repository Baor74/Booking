...
SELECT p.*
FROM Phong p
         INNER JOIN Khach_san k ON p.Ma_khach_san = k.Ma_khach_san
WHERE k.Ten_khach_san = N'Khách sạn Sài Gòn'

SELECT phong
FROM Phong AS phong
         INNER JOIN Khach_san AS khanhsan ON phong.Ma_khach_san = khanhsan.Ma_khach_san
WHERE khanhsan.Ten_khach_san = N'Khách sạn Sài Gòn'